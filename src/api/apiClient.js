...
const handleUnauthorized = async (error) => {
  ...
  try {
    ...
    const logoutReason =
      ...
      lowerMessage.includes("device mismatch")
        ? "device_mismatch"
        : "session_expired";
    ...
    if (logoutCallback && typeof logoutCallback === "function") {
      ...
      console.warn(
        "⚠️ No logout callback registered. Session cleared but logout not triggered.",
      );
    }
  } catch (error) {
    ...
    try {
      ...
      if (__DEV__)
        console.warn("❌ Error tearing down location tracking:", teardownError);
    }
    await clearSession();
    if (logoutCallback && typeof logoutCallback === "function") {
      await logoutCallback();
  ...
  async (config) => {
    try {
      ...
      if (token && !isPublicEndpoint) {
        ...
        console.warn(
          `⚠️ No auth token available for ${config.method?.toUpperCase()} ${config.url}`,
        );
      }
      ...
      const listEndpoints = [
        ...
        "/collection",
        "/loan/nip",
        "/expense-category/active/list",
      ];
      const isListRequest =
        (config.method || "get").toLowerCase() === "get" &&
        listEndpoints.some((ep) => config.url && config.url.includes(ep));
      const isCollectionHistory =
        config.url && config.url.includes("/collection/history");
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 || error.response?.status === 403) {
      console.log(
        `🔐 Authentication error (${error.response?.status}) detected. Initiating logout...`,
      );
